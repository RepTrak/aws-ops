...
`latest.json` is committed pointing at the demo snapshot. When you run a real export, the
script overwrites `latest.json` locally — run the command below once after cloning so that
change never shows up in `git status`:
...
- See the repository layout section above for the `git update-index --skip-worktree` note
  on `latest.json`.
